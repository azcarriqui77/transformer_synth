...
def get_predictions(timestamp: str, temperatures: pd.DataFrame, cfg: dict) -> bool:
    ...
    if csv_filename:
        ...
        if start_date <= query_time <= end_date:
            ...
            return get_predictions(query_time, temperatures, cfg) if generate_predictions(query_time, cfg) else empty_predictions
    else:
        # Generamos nuevas predicciones:
        # En caso de que la generación de predicciones falle no devolvemos nada:
...
def generate_predictions(query_time: pd.Timestamp, cfg: dict):
    ...
    if len(csv_files) < days_to_load:
        ...
        prediction.to_csv(os.path.join(
            cfg['folder'], file_name), index=True, index_label='timestamp')
        return True
